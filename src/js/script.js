...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const movieSection = document.querySelector(".l-mv");

  window.addEventListener("scroll", function () {
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
    const movieSectionTop =
      movieSection.getBoundingClientRect().top + window.pageYOffset;

    // .l-mvセクションを超えたら表示
    if (scrollTop >= movieSectionTop + movieSection.offsetHeight) {
